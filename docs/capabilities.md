...
| **Legendary actions** | ✅ Resolved | 30 monsters carry them in the data; `advance_monster_turn(legendary=True, actor_id=...)` spends one legendary action from a per-day pool reset at the start of the creature's own turn (`LegendaryActionUsed`, C18). Utility-only actions and one costing more than 1 use are never selected by the built-in AI, and the pool size defaults to 3 for every bundled monster (neither is typed in the corpus, BACKLOG.md). |
...
| Recharge (5–6) abilities | ✅ Resolved | A 1d6 rolled at the start of the bearer's turn for each spent recharge action (`RechargeRolled`, C18); a live recharge action ranks first in `rank_monster_actions`. Recharge does not persist across combats (BACKLOG.md). |
| Regeneration | ✅ Resolved | Heals at the start of the bearer's turn up to `hp_max` while at 1+ HP (C18); unconditional per SRD 5.2 (no 2014-style acid/fire suppression to model); a Swarm bearer never regains HP. |
| `special_abilities` | ⚠️ Partial | Typed as `MonsterTraitMechanic` on 14 trait names (C22); **Magic Resistance** grants save advantage against spells. C18 adds Pack Tactics (attack advantage from an adjacent, non-Incapacitated ally), Sunlight Sensitivity (attack disadvantage via `GridScene.sunlight`), Undead Fortitude (a CON save to hold at 1 HP instead of dropping to 0, live-path included), Swarm (no HP or temp HP gain) and Legendary Resistance. Flyby, Nimble Escape, Keen Senses and Aggressive remain unconsumed — untyped or absent from the corpus (BACKLOG.md) |
