...
| Conditions (the 15 SRD conditions) | ✅ Resolved | Applied/removed and gated by immunities. Enforced on the live path: attack-roll advantage/disadvantage for every SRD 5.2 row incl. Prone (distance-aware) and Grappled (grappler-aware); Incapacitated (and Paralyzed/Stunned/Petrified/Unconscious) rejects action/bonus/reaction intents with `IntentRejectedError("actor_incapacitated")` and skips reactions; Speed 0 (`MoveFailed(reason="speed_zero")`); auto-failed STR/DEX saves and Restrained DEX disadvantage on every save path; Paralyzed/Unconscious auto-crit within 5 ft; Charmed cannot attack or harmfully target the charmer; Characters fall Unconscious at 0 HP (massive damage kills). **Not enforced:** Frightened line-of-sight gate / can't-approach, Blinded/Deafened sense-based check auto-fail, Invisible's "can somehow see you" carve-out, Incapacitated's concentration break and initiative disadvantage (C13/C14/C16b). |
