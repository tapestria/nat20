...
| Turn lifecycle — start/end of turn, top-of-round hooks | ⚠️ Partial | The seam exists (`turn_lifecycle.py`, run off the single advance path in registration order); the registered hooks are the round-duration tick, the timed-effect expiry (`seconds` / `turns` / until-end-of-next-turn) and the reaction-effect expiry. A driven monster's own turn additionally runs legendary-pool reset, recharge rolls (`RechargeRolled`) and regeneration at its head (C18, `_run_monster_turn_start`) — ongoing damage (Acid Arrow, Spirit Guardians) still has no producer |
...
| HP, AC, hit dice, skill/save proficiencies | ✅ Resolved | HP folds each class's Fixed Hit Points (or recorded rolls) plus the current CON modifier per level, and always-on bonuses such as Dwarven Toughness and Draconic Resilience (R5). AC picks the highest eligible SRD mode — Unarmored Defense, Draconic Resilience, Mage Armor, or default — from worn armor and Shield training when left unset (R8). Save, skill and weapon/armor-training proficiencies come from class, subclass, species and background `Trait` grants, scoped to the class that grants them (R7). An explicit `CombatInstance` value always wins |
...
| **Multiclassing** | ⚠️ Partial | `CharacterBuildSpec.classes` carrier + multiclass/Pact slot-table derivation (C17); the sheet derives per-class features, HP, hit dice, proficiencies and non-stacking Extra Attack (C19), while live combat still resolves features from the primary class at total level (BACKLOG.md) |
| Feats | ⚠️ Partial | 1 of 17 corpus feats carries a mechanical activity. Prerequisites are checked and feats are recorded by `derive_sheet`; only the Ability Score Improvement feat's increases apply (C19, BACKLOG.md) |
