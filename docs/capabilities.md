...
| Turn lifecycle — start/end of turn, top-of-round hooks | ⚠️ Partial | The seam exists (`turn_lifecycle.py`, run off the single advance path in registration order); the registered hooks are the round-duration tick, the timed-effect expiry (`seconds` / `turns` / until-end-of-next-turn), the reaction-effect expiry and Rage's end-of-turn extension check (C20). A driven monster's own turn additionally runs legendary-pool reset, recharge rolls (`RechargeRolled`) and regeneration at its head (C18, `_run_monster_turn_start`) — ongoing damage (Acid Arrow, Spirit Guardians) still has no producer |
...
| Ability & skill checks (in and out of combat) | ✅ Resolved | `resolve_check`; seed via `CheckSpec.rng`; `CheckSpec.jack_of_all_trades` / `reliable_talent` (C19); in-combat checks do not read them yet (BACKLOG.md) |
| Action economy (action, bonus action, reaction, movement) | ⚠️ Partial | Extra Attack (`attacks_remaining` per Action, `LiveCombatView.turn`) and two-weapon fighting (Light-property off-hand Bonus Action) are modelled (C14). Action Surge's extra action (C20: `LiveCombatView.turn.extra_actions_remaining`; anything but a Magic action, where every `cast_spell`, `use_item` and `use_feature` counts as one; one surge per turn), Flurry of Blows' two Unarmed Strikes (three with Heightened Focus; they are the next `unarmed-strike` attacks after the committed Flurry, with no flag), Martial Arts' Bonus Unarmed Strike (`use_bonus_action=True` on an `unarmed-strike` attack: the Bonus Action, turn kept) and Cunning Action's Bonus-Action Dash and Disengage are modelled too. A one-attack actor's turn still ends after its Attack action, so a surge or a Flurry has to come first (BACKLOG.md); strikes a Flurry still owes keep the turn open. Incapacitated blocks action/bonus/reaction intents (C12). |
| Dash, Disengage | ✅ Resolved | The Action by default; the Bonus Action through Cunning Action (`use_bonus_action=True`), read from the granted features at each class's own level — a Rogue 2 or any build with two Rogue levels, not `class_slug == "rogue"` (C20) |
...
| Grapple / Shove | ⚠️ Partial | Unarmed Strike options (C14): the target saves STR or DEX, whichever modifier is higher (tie → STR), vs DC `8 + STR mod + PB` (DEX instead of STR when higher, for a creature with Martial Arts active — C20); Grapple applies the Grappled condition with the escape DC stored on it, Shove applies Prone (default) or a 5-ft push per `PlayerIntent.shove_push`. Size gate, free-hand gate, and distance-exceeded auto-release are not modelled (BACKLOG.md). |
| Two-weapon fighting | ✅ Resolved | A Light main-hand weapon opens a same-turn off-hand Bonus Action swing with a different Light weapon (C14); the ability modifier is added to that damage only if negative — or always, with the Two-Weapon Fighting style (C20). |
...
| **Lair actions** | ❌ Not modelled | Blocked on translator support — the corpus ships zero `lair_actions`; the initiative-20 pseudo-turn is designed but unimplemented (BACKLOG.md) |
| Recharge (5–6) abilities | ✅ Resolved | A 1d6 rolled at the start of a living bearer's turn for each spent recharge action (`RechargeRolled`, C18); a live recharge action ranks first in `rank_monster_actions`. Recharge does not persist across combats (BACKLOG.md). |
| Regeneration | ✅ Resolved | Heals at the start of the bearer's turn up to `hp_max` while at 1+ HP (C18); unconditional, because the bundled corpus text (one shared trait for the troll, troll limb, oni and shield guardian) carries no suppression clause; a Swarm bearer never regains HP. |
| `special_abilities` | ⚠️ Partial | Typed as `MonsterTraitMechanic` on 14 trait names (C22); **Magic Resistance** grants save advantage against spells. C18 adds Pack Tactics (attack advantage from an adjacent, non-Incapacitated ally), Sunlight Sensitivity (attack disadvantage via `GridScene.sunlight`), Undead Fortitude (a CON save to hold at 1 HP instead of dropping to 0, live-path included), Swarm (no HP or temp HP gain) and Legendary Resistance (a per-day pool from the typed `Monster.legendary_resistance_uses` — 3, 4 or 6 — armed by the host with `resolve_legendary_resistance`). Flyby, Nimble Escape, Keen Senses and Aggressive remain unconsumed — untyped or absent from the corpus (BACKLOG.md) |
...
| Mechanic | Status | Notes |
|---|---|---|
| Class, subclass, level 1–20, species | ✅ Resolved | a subclass is rejected below its class's `Subclass` advancement level (3 for every SRD 5.2 class), checked at the owning class's level (C19) |
| Background | ⚠️ Partial | `background_slug` grants the background's skill proficiencies; its +2/+1 or +1/+1/+1 adjustment applies through a `background:` choice token (C19). Tool proficiency and Origin feat not applied (BACKLOG.md) |
| HP, AC, hit dice, skill/save proficiencies | ✅ Resolved | HP folds each class's Fixed Hit Points (or recorded rolls) plus the current CON modifier per level, and always-on bonuses such as Dwarven Toughness and Draconic Resilience. AC picks the highest eligible SRD mode — Unarmored Defense, Draconic Resilience, Mage Armor, or default — from worn armor and Shield training when left unset. Save, skill and weapon/armor-training proficiencies come from class, subclass, species and background `Trait` grants, scoped to the class that grants them. An explicit `CombatInstance` value always wins |
| Ability scores, proficiency, expertise | ⚠️ Partial | The Ability Score Improvement feat's increases and the background's `background:` adjustment both apply, each capped at 20; `ability_score_method` (`standard_array` or `point_buy`) is checked against the base scores; Expertise requires proficiency in the skill (C19). Other feats' ability increases and level-20 capstone increases are not applied (BACKLOG.md) |
| Class/species feature activities (Rage, Second Wind, …) | ⚠️ Partial | Data-driven where the corpus carries a typed activity. Extra Attack is read from granted feature slugs (C14: `extra-attack` / `two-extra-attacks` / `three-extra-attacks`, highest tier wins, never summed). Per-rest caps evaluate every corpus `uses.max` shape — `@prof`, `max(1, @abilities.<ab>.mod)`, `N * @classes.<class>.levels`, `@scale.*` — and each invocation spends its activity's own `consumption.targets` cost (the free Patient Defense costs nothing, Remove Poison 5); Lay on Hands' Heal draws `PlayerIntent.pool_points` from its pool (C20). The class mechanics the engine applies itself are in the rows below. **Still prose-only, so inert:** Divine Smite, Metamagic / Sorcery Points, Eldritch Invocations; Cunning Action's own activities do nothing, and Remove Poison doesn't end Poisoned (BACKLOG.md) |
| Fighting Style feats | ✅ Resolved | The four SRD 5.2 styles, applied by the engine by feat slug (C20): Defense +1 AC in `derive_sheet` while Light, Medium or Heavy armor is worn (a hand-built `PartyMemberSpec`'s `ac` is taken as given); Archery +2 to Ranged-weapon attack rolls, on top of a pinned `attack_bonus`; Great Weapon Fighting treats a 1 or 2 on a damage die as a 3 (a Melee weapon that is Two-Handed, or Versatile held in two hands); Two-Weapon Fighting adds the ability modifier to the Light off-hand swing. Read from `DerivedSheet.feats` (forwarded by `build_party_member`) or `PartyMemberSpec.feats` / `fighting_style`. Opportunity attacks apply none (BACKLOG.md) |
...
| Rage | ⚠️ Partial | Lasts until the end of the barbarian's next turn; an attack roll against an enemy, an enemy's saving throw during that turn, or `use_feature rage` again as a Bonus Action (no use spent) extends it, and the Incapacitated condition ends it at once — `EffectExpired` with `reason="not_extended"` or `"incapacitated"` (C20). Heavy armor, "No Concentration or Spells" and the 10-minute cap are not modelled; the corpus effect's 10 rounds stay the outer cap (BACKLOG.md) |
...
| **Multiclassing** | ⚠️ Partial | `CharacterBuildSpec.classes` carrier + multiclass/Pact slot-table derivation (C17); the sheet derives per-class features, HP, hit dice, proficiencies and non-stacking Extra Attack (C19); live combat reads `PartyMemberSpec.classes` (C20): each class's features and scale values at its own level, a subclass at its class's level, `@classes.<class>.levels` per class. A host that sends no `classes` keeps the single-class reading. The spellcasting ability still comes from `class_slug`, and picked features (Invocations, Blessed Warrior) never reach the live feature gate (BACKLOG.md) |
| Feats | ⚠️ Partial | The four SRD 5.2 Fighting Style feats apply (C20, row above); of the other 13 corpus feats, 1 carries a mechanical activity. Prerequisites are checked and feats are recorded by `derive_sheet`; only the Ability Score Improvement feat's increases apply (C19, BACKLOG.md) |
